...
void dwcopy_config_write_to_disk(const DqnIni *const ini, DqnPushBuffer *const buffer);

// Does heavylifting to load the config file into internal representation in one function
// Returns a DwcopyLocation with allocated memory from buffer permanently
...
DqnIni *const  dwcopy_config_load_to_ini      (DqnPushBuffer *const buffer, const bool needWritePermission, DqnFile *const fileHandle);
void           dwcopy_config_load_to_ini_close(DqnIni *const ini, DqnFile *const fileHandle);
